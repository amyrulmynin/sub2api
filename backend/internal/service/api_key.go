...
type APIKey struct {
	...
	// Quota fields
	Quota     float64    // Quota limit in MYR (0 = unlimited)
	QuotaUsed float64    // Used quota amount
	ExpiresAt *time.Time // Expiration time (nil = never expires)

	// Rate limit fields
	RateLimit5h   float64    // Rate limit in MYR per 5h (0 = unlimited)
	RateLimit1d   float64    // Rate limit in MYR per 1d (0 = unlimited)
	RateLimit7d   float64    // Rate limit in MYR per 7d (0 = unlimited)
