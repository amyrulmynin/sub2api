...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      `${t('admin.usage.inputCost')} (MYR)`, `${t('admin.usage.outputCost')} (MYR)`,
      `${t('admin.usage.cacheReadCost')} (MYR)`, `${t('admin.usage.cacheCreationCost')} (MYR)`,
      t('usage.rate'), t('usage.accountMultiplier'), `${t('usage.original')} (MYR)`, `${t('usage.userBilled')} (MYR)`, `${t('usage.accountBilled')} (MYR)`,
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.input_cost ?? 0, log.output_cost ?? 0,
        log.cache_read_cost ?? 0, log.cache_creation_cost ?? 0,
        log.rate_multiplier?.toPrecision(4) || '1.00', (log.account_rate_multiplier ?? 1).toPrecision(4),
        log.total_cost ?? 0, log.actual_cost ?? 0,
        (log.account_stats_cost ?? log.total_cost) * (log.account_rate_multiplier ?? 1), log.first_token_ms ?? '', log.duration_ms,
