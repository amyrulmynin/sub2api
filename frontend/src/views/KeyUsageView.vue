              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ formatCurrency(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ formatCurrency(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const ringItems = computed<RingItem[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      ...
      items.push({ title: t('keyUsage.totalQuota'), pct, amount: `${formatCurrency(data.quota.used)} / ${formatCurrency(data.quota.limit)}`, iconType: 'dollar' })
    }
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        items.push({
          ...
          pct,
          amount: `${formatCurrency(rl.used)} / ${formatCurrency(rl.limit)}`,
    ...
    if (data.subscription) {
      ...
      for (const l of limits) {
        if (l.limit != null && l.limit > 0) {
          ...
          items.push({ title: l.label, pct, amount: `${formatCurrency(l.usage)} / ${formatCurrency(l.limit)}`, iconType: 'calendar' })
        }
      }
    }
    if (!data.subscription && data.balance != null) {
      items.push({ title: t('keyUsage.walletBalance'), pct: 0, amount: formatCurrency(data.balance), isBalance: true, iconType: 'dollar' })
    }
  }
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      ...
      rows.push({
        ...
        label: t('keyUsage.remainingQuota'), value: formatCurrency(data.quota.remaining), valueClass: remainColor,
      })
    }
    if (data.expires_at) {
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        let valueStr = `${formatCurrency(rl.used)} / ${formatCurrency(rl.limit)}`
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '日' : 'D'})`, value: `${formatCurrency(sub.daily_usage_usd)} / ${formatCurrency(sub.daily_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '周' : 'W'})`, value: `${formatCurrency(sub.weekly_usage_usd)} / ${formatCurrency(sub.weekly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '月' : 'M'})`, value: `${formatCurrency(sub.monthly_usage_usd)} / ${formatCurrency(sub.monthly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.expires_at) {
        rows.push({
    ...
    rows.push({
      iconBg: 'bg-emerald-500/10', iconColor: 'text-emerald-500', iconSvg: ICON_SHIELD,
      label: t('keyUsage.remainingQuota'), value: data.remaining != null ? formatCurrency(data.remaining) : '-', valueClass: remainColor,
    })
  }
...
const usageStatCells = computed<StatCell[]>(() => {
  ...
  return [
    ...
    { label: t('keyUsage.todayCost'), value: formatCurrency(today.actual_cost) },
    ...
    { label: t('keyUsage.totalCost'), value: formatCurrency(total.actual_cost) },
