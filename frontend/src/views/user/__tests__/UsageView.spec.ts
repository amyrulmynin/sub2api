...
describe('user UsageView', () => {
  ...
  it('exports csv with current filters and without admin-only fields', async () => {
    ...
    expect(csvContent.slice(1)).toBe([
      'Time,API Key Name,Model,Reasoning Effort,Inbound Endpoint,IP Address,Type,Billing Mode,Input Tokens,Output Tokens,Cache Read Tokens,Cache Creation Tokens,Rate Multiplier,Billed Cost (MYR),Original Cost (MYR),First Token (ms),Duration (ms)',
    ...
    expect(csvContent).toContain('Billed Cost (MYR)')
    expect(csvContent).toContain('Original Cost (MYR)')
