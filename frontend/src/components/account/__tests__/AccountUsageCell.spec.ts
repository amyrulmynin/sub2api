...
describe('AccountUsageCell', () => {
  ...
  it('Key 账号会展示 today stats 徽章并带 A/U 提示', async () => {
		...
		expect(wrapper.text()).toContain('A RM12.35')
		expect(wrapper.text()).toContain('U RM6.79')
		expect(wrapper.text()).not.toContain('$12.35')
  ...
  it('Grok OAuth 会展示本地 user billed 用量并保留超限百分比', async () => {
    ...
    expect(wrapper.text()).toContain('A RM0.12')
    expect(wrapper.text()).toContain('U RM0.34')
  ...
  it('Vertex 账号会在 Gemini 用量窗口里展示 today stats 徽章', async () => {
		...
		expect(wrapper.text()).toContain('A RM0.00')
		expect(wrapper.text()).toContain('U RM0.00')
  })
