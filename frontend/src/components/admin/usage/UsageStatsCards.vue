      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-orange-500">{{ t('usage.accountCost') }} {{ formatCurrency(totalAccountCost) }}</span>
            <span> · </span>
          </template>
          <span>
            {{ t('usage.standardCost') }}
            <span :class="{ 'line-through': strikeStandardCost }">{{ formatCurrency(stats?.total_cost) }}</span>
          </span>
        </p>
      </div>
    </div>
